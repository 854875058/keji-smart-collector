...
import { Search, FolderOpen, Sparkles, Home, Settings } from 'lucide-react'

type View = 'home' | 'snippets' | 'agent'
      ...
      <nav className="flex items-center gap-1 border-b border-slate-200 bg-white px-3 py-2">
        <button
          ...
          className={`flex items-center gap-1.5 rounded-lg px-3 py-1.5 text-sm font-medium transition-colors ${
            view === 'home' ? 'bg-emerald-50 text-emerald-700' : 'text-slate-600 hover:bg-slate-100'
          }`}
        >
        ...
        <button
          ...
          className={`flex items-center gap-1.5 rounded-lg px-3 py-1.5 text-sm font-medium transition-colors ${
            view === 'snippets' ? 'bg-emerald-50 text-emerald-700' : 'text-slate-600 hover:bg-slate-100'
          }`}
        >
  ...
  return (
    <div className="p-6 space-y-6">
      <div className="text-center">
        <h1 className="text-2xl font-bold text-slate-900">可记</h1>
        <p className="text-sm text-slate-500 mt-1">智能收藏助手</p>
      </div>

      <div className="grid grid-cols-2 gap-3">
        <div className="rounded-xl border border-slate-200 bg-white p-4 text-center">
          <div className="text-2xl font-bold text-emerald-600">{snippetCount}</div>
          <div className="text-xs text-slate-500">笔记</div>
        </div>
        <div className="rounded-xl border border-slate-200 bg-white p-4 text-center">
          <div className="text-2xl font-bold text-emerald-600">{folderCount}</div>
          <div className="text-xs text-slate-500">笔记本</div>
        </div>
      </div>
